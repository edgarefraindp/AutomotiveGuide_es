/**
 * @file waveshare_rgb_lcd_port.c
 * @brief Waveshare RGB LCD panel porting for ESP32-S3
 *
...
// Replace the draw_event_label with a timer callback
static void update_label_timer(lv_timer_t *timer)
{
    lv_obj_t *label = timer->user_data;
    static int counter = 0;
    char buffer[32];
    snprintf(buffer, sizeof(buffer), "Dachshund...  %d", counter++);
...
void example_lvgl_demo_ui() // LVGL demo UI initialization function 
{
    ...
    // Create the label without the event callback
    lv_obj_t *label = lv_label_create(scr);
    lv_label_set_text(label, "Hello!");
    lv_obj_align(label, LV_ALIGN_TOP_MID, 0, 10);

    // Create a timer to update the label instead of using the draw event
    lv_timer_create(update_label_timer, 500, label);
